...
class GradientBoost(SklearnModel):
    def __init__(self, model_name='gradient boost', *args, **kwargs):
    ...
    def buildReturnModel(self):
        ...
        if self.exp_type:
            if self.exp_type == 'regression':
                model = GradientBoostingRegressor(**self.default_args)
            elif self.exp_type == 'classification':
                model = GradientBoostingClassifier(**self.default_args)
            else:
                raise ValueError('Experiment type argument must be specified for GradientBoost - sklearn!')
        else:
            raise ValueError('Experiment type argument must be specified for GradientBoost - sklearn!')
