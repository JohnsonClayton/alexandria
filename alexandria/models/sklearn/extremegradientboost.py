...
class ExtremeGradientBoost(SklearnModel):
    def __init__(self, model_name='extreme gradient boost', *args, **kwargs):
    ...
    def buildReturnModel(self):
        ...
        if self.exp_type:
            if self.exp_type == 'regression':
                model = XGBRegressor(**self.default_args)
            elif self.exp_type == 'classification':
                model = XGBClassifier(**self.default_args)
            else:
                raise ValueError('Experiment type argument must be specified for ExtremeGradientBoost - sklearn!')
        else:
            raise ValueError('Experiment type argument must be specified for ExtremeGradientBoost - sklearn!')
